...
namespace Modules.Content.Characters.Employer.Code.Controller
{
    ...
    public class ControllerEmployer : BaseController
    {
        ...
        public override void Init()
        {
            ...
            _viewEmployer.ViewKnifeSetVisibility(false);
            _viewEmployer.ViewPanSetVisibility(false);
        }
        ...
        private async UniTaskVoid ExecuteWorkflow(string pointID, ControllerCustomer controllerCustomer,
            FoodType foodType, Action onComplete)
        {
        ...
        private async UniTask WorkFlow(string pointID, ControllerCustomer controllerCustomer, FoodType foodType)
        {
            ...
            GameObject helpViewInstrument = null;

            switch (generator.FoodType)
            {
                case FoodType.Sushi:
                    helpViewInstrument = _viewEmployer.ViewKnife;
                    break;
            }

            if (helpViewInstrument != null)
                helpViewInstrument.gameObject.SetActive(true);

            await generator.StartUse(this);

            if (helpViewInstrument != null) 
                helpViewInstrument.SetActive(false);
