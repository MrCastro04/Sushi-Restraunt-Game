...
namespace Modules.Content.Characters.Employer.Code.View
{
    public class ViewEmployer : BaseView
    {
        ...
        [SerializeField] private GameObject _viewKnife;
        [SerializeField] private GameObject _viewPan;

        private float _immitationTime;
        private string ANIMATOR_PARAM_IS_CHOP_FOOD = "IsChopFood";
        private string ANIMATOR_PARAM_IS_TAKING_ORDER = "IsTakingOrder";

        [field: SerializeField] public Transform ViewFoodTransform { get; private set; }
        public GameObject ViewKnife => _viewKnife;

        public GameObject ViewPan => _viewPan;
        
        public void ViewKnifeSetVisibility(bool state) => _viewKnife.SetActive(state);
        public void ViewPanSetVisibility(bool state) => _viewPan.SetActive(state);
        ...
        public async UniTask GoToPoint(PointMono pointMono, bool withImmitation = false, bool withFood = false)
        {
            ...
            if (withImmitation)
            {
                PlayAnimationStartTakeOrder();

                await _loadingCircle.RunImmitation(_immitationTime);

                PlayAnimationEndTakeOrder();
            }
        }
        ...
        public void PlayAnimationStartTakeOrder()
        {
            _animator.SetBool(ANIMATOR_PARAM_IS_TAKING_ORDER, true);
        }

        public void PlayAnimationEndTakeOrder()
        {
            _animator.SetBool(ANIMATOR_PARAM_IS_TAKING_ORDER, false);
        }
    }
}